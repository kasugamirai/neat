...
// Network occupies top 45% of canvas, table below
const nodes = [
  { x: 0.10, y: 0.08, c: '#00d4ff', l: '1', lb: '输入' },
  { x: 0.10, y: 0.28, c: '#00d4ff', l: '2', lb: '输入' },
  { x: 0.42, y: 0.08, c: '#7b2ff7', l: '5', lb: '隐藏' },
  { x: 0.75, y: 0.18, c: '#ff6bcb', l: '3', lb: '输出' },
  { x: 0.10, y: 0.48, c: '#00d4ff', l: '4', lb: '偏置' },
]
...
function draw(step) {
  ...
  for (let i = 0; i < showCount; i++) {
    ...
    drawEdge(c, fn.x * W, fn.y * netH + 10, tn.x * W, tn.y * netH + 10, color, Math.abs(cn.w), !cn.enabled)
  }
  nodes.forEach(n => drawNode(c, n.x * W, n.y * netH + 10, R, n.c, n.l, n.lb))

  // Table area: bottom 48%
  const tableY = H * 0.55
  c.fillStyle = 'rgba(255,255,255,0.04)'
  c.fillRect(20, tableY, W - 40, H - tableY - 8)

  c.fillStyle = '#fff'
  c.font = 'bold 12px "Noto Sans SC", sans-serif'
  c.textAlign = 'left'
  c.textBaseline = 'alphabetic'
  c.fillText('📋 基因组（连接基因列表）', 30, tableY - 4)
  ...
  c.textAlign = 'center'
  headers.forEach((hdr, i) => c.fillText(hdr, startX + colW * i + colW / 2, tableY + 18))

  for (let i = 0; i < showCount; i++) {
    const cn = conns[i]
    const ry = tableY + 38 + i * 24
    if (step > 0 && i === step - 1) {
      c.fillStyle = 'rgba(0,212,255,0.1)'
      c.fillRect(25, ry - 12, W - 50, 22)
    }
...
const { ctx, w, h } = useCanvas(canvasRef, 960, 480, () => draw(currentStep))
...
watch(() => props.active, v => { if (v) setTimeout(() => draw(currentStep), 80) })
</script>
